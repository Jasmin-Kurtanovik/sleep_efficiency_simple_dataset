...
from config import (  # noqa: E402
    ...
    EVALUATION_RESULTS_DIR_TRANSFORMER,
    HIDDEN_SIZE,
    HISTORY_SAVE_DIR_TRANSFORMER,
    INPUT_SIZE,
    LEARNING_RATE,
    LR_SCHEDULER_PATIENCE,
    MAX_EPOCHS,
    MODEL_SAVE_PATH_TRANSFORMER,
    OUTPUT_SIZE,
    RANDOM_SEED,
    SEQUENCE_LENGTH,
    TRAIN_RATIO,
...
from model_transformer import TransformerModel  # noqa: E402
...
class Trainer:
    ...
    def fit(
        ...
        epochs: int,
        model_path: str = MODEL_SAVE_PATH_TRANSFORMER,
...
def main():
    """Run the training and validation pipeline for the Transformer."""
    ...
    history = trainer.fit(train_loader, val_loader, epochs=MAX_EPOCHS, model_path=MODEL_SAVE_PATH_TRANSFORMER)

    Path(MODEL_SAVE_PATH_TRANSFORMER).parent.mkdir(parents=True, exist_ok=True)
    torch.save(trainer.model.state_dict(), MODEL_SAVE_PATH_TRANSFORMER)
    if not Path(MODEL_SAVE_PATH_TRANSFORMER).exists():
        raise FileNotFoundError(f"Failed to save model checkpoint to {MODEL_SAVE_PATH_TRANSFORMER}")

    Path(HISTORY_SAVE_DIR_TRANSFORMER).mkdir(parents=True, exist_ok=True)
    history_path = Path(HISTORY_SAVE_DIR_TRANSFORMER) / f"history_transformer_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(history_path, "w", encoding="utf-8") as handle:
        json.dump(history, handle, indent=2)

    Path(EVALUATION_RESULTS_DIR_TRANSFORMER).mkdir(parents=True, exist_ok=True)

    logger.info(f"Training complete. Model saved to {MODEL_SAVE_PATH_TRANSFORMER}")
